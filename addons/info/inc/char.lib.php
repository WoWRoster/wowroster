...
class char
{
	...
	function show_quests( )
	{
		...
		$roster->tpl->set_handle('quests', $addon['basename'] . '/quests.html');
	...
	function show_recipes( )
	{
		...
		$roster->tpl->set_handle('recipes', $addon['basename'] . '/recipes.html');
	...
	function show_mailbox( )
	{
		...
		$sqlquery = "SELECT * FROM `" . $roster->db->table('mailbox') . "` " . "WHERE `member_id` = '" . $this->data['member_id'] . "' " . "ORDER BY `mailbox_days`;";
		...
		$roster->tpl->set_handle('mailbox', $addon['basename'] . '/mailbox.html');
	...
	function pet_stat( $statname , $data )
	{
		...
		$line = '<span style="color:#ffffff;font-size:11px;font-weight:bold;">' . $tooltipheader . '</span><br />' . '<span style="color:#DFB801;">' . $tooltip . '</span>';
	...
	function pet_resist( $resname , $data )
	{
		...
		$tooltip = '<span style="color:' . $color . ';font-size:11px;font-weight:bold;">' . $name . '</span> ' . $this->rating_long('res_' . $resname, $data) . '<br />' . '<span style="color:#DFB801;text-align:left;">' . $tooltip . '</span>';
	...
	function wskill( $location )
	{
		...
		if( $location == 'ranged' )
		{
			...
			$line = '<span style="color:#ffffff;font-size:11px;font-weight:bold;">' . $tooltipheader . '</span><br />' . '<div style="color:#DFB801;">' . $tooltip . '</div>';
		}
		else
		{
			...
			if( $this->data['melee_ohand_dps'] > 0 )
			{
				...
				$line .= '<br /><span style="color:#ffffff;font-size:11px;font-weight:bold;">' . $tooltipheader . '</span><br />' . '<div style="color:#DFB801;">' . $tooltip . '</div>';
			}
		}
	...
	function resilience( )
	{
		...
		$tooltipheader = $name;
		$tooltip = '<div><span style="float:right;">' . $this->data['stat_res_melee'] . '</span>' . $roster->locale->act['melee'] . '</div>' . '<div><span style="float:right;">' . $this->data['stat_res_ranged'] . '</span>' . $roster->locale->act['ranged'] . '</div>' . '<div><span style="float:right;">' . $this->data['stat_res_spell'] . '</span>' . $roster->locale->act['spell'] . '</div>';
	...
	function resist_value( $resname )
	{
		...
		$tooltip = '<span style="color:' . $color . ';font-size:11px;font-weight:bold;">' . $name . '</span> ' . $this->rating_long('res_' . $resname) . '<br />' . '<span style="color:#DFB801;text-align:left;">' . $tooltip . '</span>';
	...
	function _alt_name_hover( )
	{
		...
		{
			$sql = "SELECT `main_id` FROM `" . $roster->db->table('alts', 'memberslist') . "` WHERE `member_id` = " . $this->data['member_id'] . ";";
			...
			if( $main_id != 0 )
			{
				...
				if( isset($alts[1]) )
				{
					...
					foreach( $alts as $alt )
					{
						...
						{
							$caption = '<a href="' . makelink('char-info&amp;a=c:' . $alt['member_id']) . '">' . $alt['name'] . ' (' . $roster->locale->act['level'] . ' ' . $alt['level'] . ' ' . $alt['class'] . ')</a>';
						}
						else
						{
					...
					$alt_hover = 'style="cursor:pointer;" onmouseover="return overlib(overlib_alt_html,CAPTION,overlib_alt_cap);" ' . 'onclick="return overlib(overlib_alt_html,CAPTION,overlib_alt_cap,STICKY,OFFSETX,-5,OFFSETY,-5,NOCLOSE);" ' . 'onmouseout="return nd();"';
				}
			}
		}
	...
	function out( )
	{
		...
		$roster->tpl->set_handle('char', $addon['basename'] . '/char.html');
...
function char_get_one_by_id( $member_id )
{
	...
	$query = "SELECT a.*, b.*, `c`.`guild_name`, DATE_FORMAT(  DATE_ADD(`a`.`dateupdatedutc`, INTERVAL " . $roster->config['localtimeoffset'] . " HOUR ), '" . $roster->locale->act['timeformat'] . "' ) AS 'update_format' " . "FROM `" . $roster->db->table('players') . "` a, `" . $roster->db->table('members') . "` b, `" . $roster->db->table('guild') . "` c " . "WHERE `a`.`member_id` = `b`.`member_id` AND `a`.`member_id` = '$member_id' AND `a`.`guild_id` = `c`.`guild_id`;";
	$result = $roster->db->query($query);
	if( $roster->db->num_rows($result) > 0 )
	{
...
function char_get_one( $name , $server )
{
	...
	$query = "SELECT `a`.*, `b`.*, `c`.`guild_name`, DATE_FORMAT(  DATE_ADD(`a`.`dateupdatedutc`, INTERVAL " . $roster->config['localtimeoffset'] . " HOUR ), '" . $roster->locale->act['timeformat'] . "' ) AS 'update_format' " . "FROM `" . $roster->db->table('players') . "` a, `" . $roster->db->table('members') . "` b, `" . $roster->db->table('guild') . "` c " . "WHERE `a`.`member_id` = `b`.`member_id` AND `a`.`name` = '$name' AND `a`.`server` = '$server' AND `a`.`guild_id` = `c`.`guild_id`;";
	$result = $roster->db->query($query);
	if( $roster->db->num_rows($result) > 0 )
	{
		$data = $roster->db->fetch($result);
