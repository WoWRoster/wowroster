...
if( $type == 'guildwins' )
{
	$query = "SELECT `pvp`.`guild`, COUNT(`pvp`.`guild`) AS countg" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `members`.`guild_id` = '" . $roster->data['guild_id'] . "' AND `pvp`.`win` = '1' AND `pvp`.`enemy` = '1'" . " GROUP BY `pvp`.`guild`" . " ORDER BY countg DESC;";
...
{
	$query = "SELECT `pvp`.`guild`, COUNT(`pvp`.`guild`) AS countg" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `members`.`guild_id` = '" . $roster->data['guild_id'] . "' AND `pvp`.`win` = '0' AND `pvp`.`enemy` = '1'" . " GROUP BY `pvp`.`guild`" . " ORDER BY countg DESC;";
...
elseif( $type == 'enemywins' )
{
	...
	$query = "SELECT `pvp`.`name`, `pvp`.`guild`, `pvp`.`race`, `pvp`.`class`, `pvp`.`leveldiff`, COUNT(`pvp`.`name`) AS countg" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `members`.`guild_id` = '" . $roster->data['guild_id'] . "' AND `pvp`.`win` = '1' AND `pvp`.`enemy` = '1'" . " GROUP BY `pvp`.`name`" . " ORDER BY countg DESC, `pvp`.`leveldiff` DESC;";
...
{
	$query = "SELECT `pvp`.`member_id`, `members`.`name` AS gn, COUNT(`pvp`.`member_id`) AS countg" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `members`.`guild_id` = '" . $roster->data['guild_id'] . "' AND `pvp`.`win` = '1' AND `pvp`.`enemy` = '1'" . " GROUP BY `pvp`.`member_id`" . " ORDER BY countg DESC;";
...
elseif( $type == 'pvpratio' )
{
	...
	$query = "SELECT `members`.`name`, `members`.`member_id`, IF(`pvp`.`win` = '1', 1, 0) AS win, SUM(`pvp`.`win`) AS wtotal, COUNT(`pvp`.`win`) AS btotal" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `members`.`guild_id` = '" . $roster->data['guild_id'] . "' AND `pvp`.`leveldiff` < 8 AND `pvp`.`leveldiff` > -8 AND `pvp`.`enemy` = '1'" . " GROUP BY `members`.`name`" . " ORDER BY wtotal DESC;";
...
elseif( $type == 'playerinfo' )
{
	...
	$first = true;
	$query = "SELECT `pvp`.*, `members`.`name` AS gn" . " FROM `" . $roster->db->table('pvp2', $addon['basename']) . "` AS pvp" . " LEFT JOIN `" . $roster->db->table('members') . "` AS members ON `members`.`member_id` = `pvp`.`member_id`" . " WHERE `pvp`.`name` = '" . $player . "'";
