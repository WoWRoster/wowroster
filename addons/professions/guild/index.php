...
$query = "SELECT `s`.*, `p`.`name`, `p`.`clientLocale`, `p`.`member_id` FROM `" . $roster->db->table('skills') . "` AS s, `" . $roster->db->table('players') . "` AS p" . " WHERE `p`.`member_id` = `s`.`member_id`" . " AND `p`.`guild_id` = '" . $roster->data['guild_id'] . "'" . $showNewSkill . " AND `skill_name` IN ($inClause)" . " ORDER BY `s`.`skill_type`, (mid(`skill_level` FROM 1 FOR (locate(':', `skill_level`)-1)) + 0) DESC, `s`.`skill_name`, `p`.`name`;";
