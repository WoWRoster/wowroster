...
<?php print border('sgray','start','Main Settings'); ?>
  <table class="sc_table" cellspacing="0" cellpadding="2">
    ...
    <tr>
      ...
      <td class="sc_row_right<?php echo ((($row)%2)+1); ?>" align="right"><label>
        <input type="radio" class="checkBox" name="gif_dither" value="1" <?php print ( $configData['etag_cache'] ? 'checked="checked"' : '' ); ?> />
        Yes</label> <label>
        <input type="radio" class="checkBox" name="gif_dither" value="0" <?php print ( !$configData['etag_cache'] ? 'checked="checked"' : '' ); ?> />
        No</label></td>
    </tr>
  </table>
<?php
