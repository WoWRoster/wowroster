...
ob_start();
include_once( $sigconfig_dir.'templates/sc_passbox.tpl' );
...
{
	if( isset($_COOKIE['siggen_pass']) )
		setcookie( 'siggen_pass','',time()-86400 );
	if( isset($_COOKIE['siggen_nametest']) )
		setcookie( 'siggen_nametest','',time()-86400 );
	if( isset($_COOKIE['siggen_configname']) )
		setcookie( 'siggen_configname','',time()-86400 );
	if( isset($_COOKIE['siggen_lasttab']) )
		setcookie( 'siggen_lasttab','',time()-86400 );
	$password_message = '<span class="red">Logged Out</span><br />';
}
else
{
	if( !isset($_COOKIE['siggen_pass']) )
	{
		if( isset($_POST['pass_word']) )
		{
			...
			{
				setcookie( 'siggen_pass', $roster_conf['roster_upd_pw'] );
				$password_message = '<span class="red">Logged in:</span><span style="font-size:10px;color:#FFFFFF"> [<a href="'.$script_filename.'&amp;logout=logout">Logout</a>]</span><br />';
				$allow_login = 1;
			}
			else
			{
				$password_message = '<span class="red">Wrong password</span><br />';
			}
		}
	}
	else
	{
		$BigCookie = $_COOKIE['siggen_pass'];
...
if( isset( $_POST['config_name'] ) )
{
	...
	setcookie( 'siggen_configname',$config_name );
}
...
if( isset( $_POST['name_test'] ) )
{
	...
	setcookie( 'siggen_nametest',$name_test );
}
