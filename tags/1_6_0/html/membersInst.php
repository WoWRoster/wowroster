<?php
$versions['versionDate']['membersInst'] = '$Date: 2006/01/02 01:34:42 $'; 
$versions['versionRev']['membersInst'] = '$Revision: 1.8 $'; 
$versions['versionAuthor']['membersInst'] = '$Author: zanix $';
...
function tableHeaderRow($th) {
	...
	foreach ($th as $header) {
		...
		if($items[$header]) {
			...
			$header = '<a href="'.$itemlink[$roster_lang].urlencode(utf8_decode($iname)).'" target="_thottbot">'.$header.'</a>';
		}
		if ($acount == 1) {
			print '    <th class="rankbordercenterleft"><div class="membersHeader">'.$header.'</div></th>'."\n";
		} elseif ($acount == count($th)) {
			print '    <th class="rankbordercenterright" align="center"><div class="membersHeaderRight">'.$header.'</div></th>'."\n";
		} else {
...
while ($row = mysql_fetch_array($result)) {
	...
	foreach ($items as $key => $data) {
		...
		if ($krow[$key] == '-2') {
			...
			print '<a href="'.$itemlink[$roster_lang].urlencode(utf8_decode($iname)).'" target="_thottbot">';
