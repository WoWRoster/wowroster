...
if ($row) {
	...
	++$striping_counter;
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=guildlosses">'.$wordings[$roster_lang]['pvplist2'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	if ($row['guild'] == '') {
		$guildname = '(unguilded)';
	} else {
...
$query = "SELECT name, COUNT(name) as countg FROM `".ROSTER_PVP2TABLE."` WHERE win = 'Yes' AND enemy = 'Yes' GROUP BY name ORDER BY countg DESC";
$result = mysql_query($query) or die(__LINE__.":".mysql_error());
$row = mysql_fetch_assoc( $result );
...
if ($row) {
	...
	++$striping_counter;
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=enemywins">'.$wordings[$roster_lang]['pvplist3'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['name']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
...
$query = "SELECT name, COUNT(name) as countg FROM `".ROSTER_PVP2TABLE."` WHERE win = 'No' AND enemy = 'Yes' GROUP BY name ORDER BY countg DESC";
$result = mysql_query($query) or die(__LINE__.":".mysql_error());
$row = mysql_fetch_assoc( $result );
...
if ($row) {
	...
	++$striping_counter;
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=enemylosses">'.$wordings[$roster_lang]['pvplist4'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['name']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
...
if ($row) {
	...
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=purgewins">'.$wordings[$roster_lang]['pvplist5'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['gn']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
...
if ($row) {
	...
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=purgelosses">'.$wordings[$roster_lang]['pvplist6'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['gn']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
...
$query = "SELECT pvp2.member_id, members.name as gn, AVG(pvp2.`diff`) as ave, COUNT(pvp2.member_id) as countg FROM `".ROSTER_PVP2TABLE."` pvp2 LEFT JOIN `".ROSTER_MEMBERSTABLE."` members ON members.member_id = pvp2.member_id WHERE win = 'Yes' AND enemy = 'Yes' GROUP BY pvp2.member_id ORDER BY ave DESC";
$result = mysql_query($query) or die(__LINE__.":".mysql_error());
$row = mysql_fetch_assoc( $result );
...
if ($row) {
	...
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=purgeavewins">'.$wordings[$roster_lang]['pvplist7'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['gn']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
...
$query = "SELECT pvp2.member_id, members.name as gn, AVG(pvp2.`diff`) as ave, COUNT(pvp2.member_id) as countg FROM `".ROSTER_PVP2TABLE."` pvp2 LEFT JOIN `".ROSTER_MEMBERSTABLE."` members ON members.member_id = pvp2.member_id WHERE win = 'No' AND enemy = 'Yes' GROUP BY pvp2.member_id ORDER BY ave DESC";
$result = mysql_query($query) or die(__LINE__.":".mysql_error());
$row = mysql_fetch_assoc( $result );
...
if ($row) {
	...
	++$striping_counter;
	pvprankRight((($striping_counter % 2) +1));
	print('<a href="indexPvp.php?type=purgeavelosses">'.$wordings[$roster_lang]['pvplist8'].'</a></td>');
	pvprankMid((($striping_counter % 2) +1));
	print($row['gn']);
	print('</td>');
	pvprankLeft((($striping_counter % 2) +1));
