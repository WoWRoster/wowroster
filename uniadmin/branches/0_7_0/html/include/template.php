...
class Template
{
	...
	function set_template($template = '', $root_dir = '')
	{
		if ( $root_dir == '' )
		{
			$this->root = UA_THEMEDIR . $template;
		}
		else
		{
			$this->root = UA_BASEDIR . $root_dir . $template;
		}

		$this->cachedir = UA_CACHEDIR . $template . DIR_SEP;
		...
		if (!file_exists($this->cachedir))
		{
			...
			if ( !@mkdir($this->cachedir, 0777) )
			{
				...
				$cachedir = str_replace(UA_BASEDIR, '/', $this->cachedir);
	...
	function compile_cache_show(&$template)
	{
		...
		$dp = opendir(UA_CACHEDIR . $template . DIR_SEP);
		while ($file = readdir($dp))
		{
			if (strstr($file, '.html') && is_file(UA_CACHEDIR . $template . DIR_SEP . $file))
			{
