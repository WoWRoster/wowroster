...
class phpNuke extends CMS_Sync
{
	
	var $cms_users_table    = 'nuke_users';
	var $cms_username_field = 'username';
	...
	var $cms_new_user_query = 'INSERT INTO nuke_users (name, username, user_password, user_email) VALUES (\'%1$s\', \'%1$s\', \'%2$s\', \'%3$s\')';
	...
	function transfer_accounts_from_cms($accounts=array())
	{
		...
		{
			if(@empty($user_info->password)) continue;
