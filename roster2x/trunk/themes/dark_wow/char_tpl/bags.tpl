...
<div class="{$bagarray[bag].type}" style="float:left;margin:3px;" id="{$bagarray[bag].id}" align="Left">
...
{if $bagarray[bag].items[bagitem].image == ""}
		<div class="bagitem"><img src="{$roster_conf.imagepath}/spacer.gif" class="bagitem_noicon" alt="" /></div>
{else}
		<div class="bagitem">
			...
			<img onmouseover="overlib({$bagarray[bag].id}_{$bagarray[bag].items[bagitem].id})"; onclick="overlib({$bagarray[bag].id}_{$bagarray[bag].items[bagitem].id}_link,STICKY,MOUSEOFF);" onmouseout="return nd();" src="{$roster_conf.iconpath}/{$bagarray[bag].items[bagitem].image}" class="bagitem_icon" alt="" /></div>
{/if}
{/section}
