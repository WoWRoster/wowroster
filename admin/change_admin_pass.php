...
if ( array_key_exists('oldpass',$_POST))
{
	...
	{
		$success = 0;
		if ( md5($_POST['oldpass']) != $roster_conf['roster_admin_pw'] )
		{
		...
		elseif ( md5($_POST['newpass1']) == $roster_conf['roster_admin_pw'] )
		{
			$body = messagebox('Password not changed. The new password was the same as the old one.','Roster Control Panel','sred');
		}
		else // valid password
		{
			$query = 'UPDATE `'.$wowdb->table('config').'` SET `config_value` = "'.md5($_POST['newpass1']).'"  WHERE `config_name` = "roster_admin_pw"';
