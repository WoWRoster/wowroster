...
$script_filename = getlink('&amp;file=addon&amp;roster_addon_name=siggen');
...
{
	if( $functions->makeDir( BASEDIR.$checkData['save_images_dir'] ) )
	{
...
{
	if( $functions->checkDir( BASEDIR.$checkData['save_images_dir'],1,1 ) )
	{
...
{
	if( !$functions->checkDir( BASEDIR.$checkData['save_images_dir'] ) )
	{
		$functions->setMessage("Saved Signatures Folder doesn't exist<br />It is required when &quot;Save Image Mode&quot; is turned on<br />Click <a href=\"$script_filename&amp;make_dir=save\">HERE</a> to try to create [<span class=\"green\">".BASEDIR.$checkData['save_images_dir']."</span>]<br />Save Image functions are temporarily disabled");
		$allow_save = false;
	}
	elseif( !$functions->checkDir( BASEDIR.$checkData['save_images_dir'],1 ) )
	{
		$functions->setMessage("Saved Signatures Folder isn't writable<br />Write access is required when &quot;Save Image Mode&quot; is turned on<br />Click <a href=\"$script_filename&amp;make_dir=chmodsave\">HERE</a> to try to chmod [<span class=\"green\">".BASEDIR.$checkData['save_images_dir']."</span>]<br />Save Image functions are temporarily disabled");
		$allow_save = false;
	}
	else
	{
