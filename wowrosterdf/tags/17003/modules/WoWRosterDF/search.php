...
<form action="<?php echo getlink(); ?>">
<input type="hidden" name="name" value="<?php echo $module_name; ?>" />
<input type="hidden" name="file" value="indexsearch" />
...
<p style="color:white;text-align;center">
  <a href="'.getlink('&amp;file=indexsearch'.($inputbox_value ? '&amp;s='.$inputbox_value.'#items' : '#items' )).'">'.$wordings[$roster_conf['roster_lang']]['items'].'</a>
  - <a href="'.getlink('&amp;file=indexsearch'.($inputbox_value ? '&amp;s='.$inputbox_value.'#recipes' : '#recipes' )).'">'.$wordings[$roster_conf['roster_lang']]['recipes'].'</a>
</p><br /><br />';
  ...
  <tr>
	...
	if( $wowdb->num_rows($result) != 0 )
	{
		...
		{
			$char_url = '&amp;file=char&amp;cname='.$data['name'].'&amp;server='.$data['server'];
  ...
  <tr>
    <td colspan="2" class="membersHeaderRight"><a href="'.getlink($char_url).'">'.$data['name'].'</a></td>
  </tr>';
			}
...
<tr>
  <td>';
	...
	//name | server | member_id | recipe_name | skill_name | difficulty | reagents | recipe_texture | recipe_tooltip
		while ($data = $wowdb->fetch_assoc( $result ))
		{
			$char_url = '&amp;file=char&amp;cname='.$data['name'].'&amp;server='.$data['server'].'&amp;action=recipes';
			if ( $cid != $data['member_id'] )
			{
				if ( $cid != '' )
				{
  ...
  <tr>
    <td colspan="3" class="membersHeaderRight"><a href="'.getlink($char_url).'">'.$data['name'].'</a></td>';
			}
