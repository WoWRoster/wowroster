...
$dffile_name = (isset($_GET['file']) ? $_GET['file'] : 'index' );

if( $roster_conf['index_update_inst'] )
{
	print '            <a href="'.getlink('&amp;file='.$dffile_name.'#update').'"><font size="4">'.$wordings[$roster_conf['roster_lang']]['update_link'].'</font></a><br /><br />';
}
...
<a name="update"></a>';
	...
	echo '<div align="left">'.$wordings[$roster_conf['roster_lang']]['update_instruct'];
...
function name_value ( $row )
{
	...
	if( $roster_conf['index_member_tooltip'] )
	{
		...
		if ( $row['server'] )
		{
			return $tooltip.'<a href="'.getlink('&amp;file=char&amp;cname='.$row['name'].'&amp;server='.$row['server']).'">'.$row['name'].'</a></div>';
		}
		else
		{
	...
	else
	{
		if ( $row['server'] )
		{
			return '<a href="'.getlink('&amp;file=char&amp;cname='.$row['name'].'&amp;server='.$row['server']).'">'.$row['name'].'</a>';
		}
		else
		{
			return $row['name'];
