...
$option_blockzones = selectQuery("`".ROSTER_QUESTSTABLE."` quests,`".ROSTER_MEMBERSTABLE."` members WHERE quests.member_id = members.member_id","DISTINCT quests.zone","zone",$zoneidsafe,"zone",getlink('&amp;file=indexquests&amp;zoneid'));
$option_blockquests = selectQuery("`".ROSTER_QUESTSTABLE."` quests,`".ROSTER_MEMBERSTABLE."` members WHERE quests.member_id = members.member_id","DISTINCT quests.quest_name","quest_name",$questidsafe,"quest_name",getlink('&amp;file=indexquests&amp;questid'));
...
print('<br /><br />
      <form method="post" action="'.getlink('&amp;file=indexquests').'">
...
if (isset($zoneidsafe) or isset($questidsafe))
{
	...
	{
		print('<br /><span class="title_text">'.$zrow['zone']."</span>\n");
    ...
    <th class="membersHeaderRight">Member</th>
			...
			while($row = $wowdb->fetch_array($result))
			{
				...
				if ($row['server'])
				{
					print('<a href="'.getlink('&amp;file=char&amp;cname='.$row['name'].'&amp;server='.$row['server']).'" target="_new">'.$row['name'].'</a>');
				}
				else
					print($row['name']);
...
if (isset($questidsafe))
{
	...
	{
		print('<br /><span class="title_text">'.$qnrow['quest_name'].'</span><br />');
