...
$optionsPanel .= '<form action="" method="GET" name="myform">
<input type="hidden" name="name" value="'.$module_name.'">
<input type="hidden" name="file" value="addon">
	<input type="hidden" name="roster_addon_name" value="gearbrowser">'."\n";
...
{	
	...
	if ($selectedRight <> "")
	{
		...
		if( !$char2 )
		{
			die_quietly('(GBC) Sorry no data in database for &quot;'.$name.'&quot; of &quot;'.$_GET['server'].'&quot;<br /><br /><a href="./index.php">'.$wordings[$roster_conf['roster_lang']]['backlink'].'</a>','Character Not Found');
		}
	}
...
{	
	...
	if (isset($_REQUEST["proffilter"]))
	{
		...
		if( isset( $recipes[0] ) )
		{
			...
			{
				$madebyContent .=  '<a href="./index.php?name='.$module_name.'&amp;file=char&amp;cname='.$row_users['name'].'&amp;server='.$server_name_escape.'&amp;action=recipes#'.$row_recipe_type['recipe_type'].'">'.$row_recipe_type['recipe_type'].'</a> - '."\n";
			}
			...
			foreach ($recipes as $recipe)
			{
				if ($recipe_type != $recipe->data['recipe_type'])
				{
					...
					$madebyContent .= border('syellow','start','<a href="./index.php?name='.$module_name.'&amp;file=char&amp;cname='.$row_users['name'].'&amp;server='.$server_name_escape.'&amp;action=recipes#top_menu" id="'.$recipe_type.'">'.$recipe_type.'</a>').
