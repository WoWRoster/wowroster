				...
				if( is_array($guild) )
				{
					...
					if( $roster_conf['guild_name'] == $guildName )
					{
						...
						if( $guild['DBversion'] >= $roster_conf['minGPver'] )
						{
							...
							$wowdb->resetMessages();
							$output .= "</ul>\n";
							$output .= "<strong>Update Summary for [<span class=\"orange\">$guildName</span>]</strong>\n<ul>\n".
								"<li>Members Updated: ".$wowdb->membersupdated."</li>\n".
								"<li>Members Added: ".$wowdb->membersadded."</li>\n".
								"</ul>\n";
						}
						else
...
if( $roster_conf['pvp_log_allow'] )
{
	$pvplogInputField = "
                    <tr>
                      <td class=\"membersRow2\" onmouseover=\"overlib('<b>PvPLog.lua</b> ".$wordings[$roster_conf['roster_lang']]['filelocation']."\\\\PvPLog.lua',WRAP,RIGHT);\" onmouseout=\"return nd();\">PvPLog.lua</td>
...
if( $htmlout )
{
	...
	if( $uploadFound )
	{
		...
		print
			...
			'<div style="font-size:10px;color:#FFFFFF;background-color:#1F1E1D;text-align:left;height:300px;width:550px;overflow:auto;">'.
				$updateMessages.
				$updatePvPMessages.
		...
		if( $roster_conf['sqldebug'] )
		{
			...
			<div style="font-size:10px;color:#FFFFFF;background-color:#1F1E1D;text-align:left;height:300px;width:560px;overflow:auto;">'.
