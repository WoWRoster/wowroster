...
class Template
{
	...
	function set_template($template = '', $root_dir = '')
	{
		...
		if (!file_exists($this->cachedir))
		{
			...
			if ( !@mkdir($this->cachedir, 0777) )
			{
				...
				$cachedir = str_replace(ROSTER_BASE, '/', $this->cachedir);
	...
	function make_filename($filename)
	{
		...
		return (substr($filename, 0, 1) != DIR_SEP) ? $this->root . DIR_SEP . $filename : $filename;
	}
	...
	function assign_from_include($filename)
	{
		...
		if (!($this->compile_load($_str, $handle, false)))
		{
			if (!$this->loadfile($handle))
			{
				trigger_error("Template->assign_from_include(): Couldn't load template file for handle $handle", E_USER_ERROR);
			}
	...
	function compile_cache_show(&$template)
	{
		...
		$dp = opendir(ROSTER_CACHEDIR . $template . DIR_SEP);
		while ($file = readdir($dp))
		{
			if (strstr($file, '.html') && is_file(ROSTER_CACHEDIR . $template . DIR_SEP . $file))
			{
