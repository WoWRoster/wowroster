...
class roster_professionsSearch
{
	...
	function roster_professionsSearch( )
	{
		...
		$this->open_table = '<tr><th class="membersHeader ts_string">' . $roster->locale->act['item'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['level'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['name'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['type'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['reagents'] . '</th>' . '<th class="membersHeaderRight ts_string">' . $roster->locale->act['character'] . '</th></tr>';
	...
	function search( $search , $limit = 10 , $page = 0 )
	{
		...
		if( $nrows > 0 && $limit > 0 )
		{
			while( $x > 0 )
			{
				...
				$item['html'] = '<td class="SearchRowCell">' . $icon->out() . '</td>' . '<td class="SearchRowCell">' . $icon->data['level'] . '</td>' . '<td class="SearchRowCell"><span style="color:#' . $icon->data['item_color'] . '">[' . $icon->data['recipe_name'] . ']</span></td>' . '<td class="SearchRowCell">' . $icon->data['skill_name'] . '<br />' . $icon->data['recipe_type'] . '</td>' . '<td class="SearchRowCell">' . str_replace('<br>', '<br />', $icon->data['reagents']) . '</td>' . '<td class="SearchRowCellRight"><a href="' . makelink('char-info-recipes&amp;a=c:' . $row['member_id'] . '#' . strtolower(str_replace(' ', '', $icon->data['skill_name']))) . '"><strong>' . $row['name'] . '</strong></a></td>';
