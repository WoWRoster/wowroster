...
if( $num_members > 0 )
{
	...
	$query = 'SELECT ' . ' `p`.`member_id`, `p`.`name`,' . ' `p`.`level`, `p`.`class`,' . ' `i`.`show_money`, `i`.`show_played`,' . ' `i`.`show_tab2`, `i`.`show_tab3`,' . ' `i`.`show_tab4`, `i`.`show_tab5`,' . ' `i`.`show_talents`, `i`.`show_spellbook`,' . ' `i`.`show_mail`, `i`.`show_bags`,' . ' `i`.`show_bank`, `i`.`show_quests`,' . ' `i`.`show_recipes`, `i`.`show_item_bonuses`' . ' FROM `' . $roster->db->table('display', $addon['basename']) . '` AS i' . ' LEFT JOIN `' . $roster->db->table('players') . '` AS p' . ' ON `p`.`member_id` = `i`.`member_id`' . ' WHERE `p`.`guild_id` = ' . $roster->data['guild_id'] . ' ORDER BY `p`.`name` ASC' . ' LIMIT ' . ($start > 0 ? $start : 0) . ', 15;';
...
$roster->tpl->set_handle('body', $addon['basename'] . '/admin/display.html');
...
function processData( )
{
	...
	if( count($update_sql) > 0 )
	{
		...
		{
			$sql = "UPDATE `" . $roster->db->table('display', $addon['basename']) . "`" . " SET " . $roster->db->build_query('UPDATE', $data) . " WHERE `member_id` = '$member_id';";
