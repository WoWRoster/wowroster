...
class vaultSearch
{
	...
	function vaultSearch( )
	{
		...
		$this->open_table = '<tr><th class="membersHeader ts_string">' . $roster->locale->act['item'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['level'] . '</th>' . '<th class="membersHeader ts_string">' . $roster->locale->act['name'] . '</th>' . '<th class="membersHeaderRight ts_string">' . $roster->locale->act['page'] . '</th></tr>';
	...
	function search( $search , $limit = 10 , $page = 0 )
	{
		...
		if( $nrows > 0 && $limit > 0 )
		{
			while( $x > 0 )
			{
				...
				$item['html'] = '<td class="SearchRowCell">' . $icon->out() . '</td>' . '<td class="SearchRowCell">' . $icon->requires_level . '</td>' . '<td class="SearchRowCell"><span style="color:#' . $icon->color . '">[' . $icon->name . ']</span></td>' . '<td class="SearchRowCellRight"><a href="' . makelink('guild-vault&amp;a=g:' . $row['guild_id']) . '">' . '<img src="' . $roster->config['interface_url'] . 'Interface/Icons/' . $tab_name[$row['guild_id']][$row['item_parent']]['icon'] . '.' . $roster->config['img_suffix'] . '" style="width:16px;height:16px;" alt="" /> ' . $tab_name[$row['guild_id']][$row['item_parent']]['name'] . '</a></td>';
